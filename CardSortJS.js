...
function loadCards(cardSpace){
	...
	for(var key in cardList){
		...
		if(cardList[key].type === 'image'){
			...
			// Load the cards into the card space.
			cardSpace.append('<div id="card' + loopy + '" class="card"><a class="cardeditlink">edit</a><p class="cardtext">' + cardList[key].text + '</p></div>');
	...
	// When a card is clicked, make it editable.
	$('.cardeditlink').on('click tap', function(){
		var text = $(this).text();
		var cardNumber = $(this).parent().attr('id').substr(4);
		var cardParagraph = $(this).parent().find('.cardtext');
	    nameChanger(cardParagraph, text, cardNumber);
	});
}
...
// Handles name changes in cards.
function nameChanger(title, text, cardNumber){

	// Turn the paragraph into a text input area.
    $(title).replaceWith('<textarea id="cardEdit"></textarea>');
    inputarea = $('#cardEdit');
    inputarea.focus();
    ...
    inputarea.keypress(function(e){
		...
		if (code == 13){
            ...
            inputarea.replaceWith('<p class="cardtext">' + inputtext + '</p>');
            JSProblemState['cards'][cardNumber] = inputtext;
...
function selfDelete(event, cardSpace){
	...
	var cardTarget = $(event.target).parent();
	var cardNumber = cardTarget.attr('id').substr(4);  // Strip off the word "card" from the id
	
	// Remove the card from JSProblemState
	delete JSProblemState['cards'][cardNumber];
...
$(document).ready(function(){
	...
	// Add a new blank card when someone clicks the plus button.
	$('#addcard').on('tap click', function(){
		//loadCards(cardSpace, 'blank');

		// Find the number of the highest existing card so we can number the new one properly.
		var cardNumbers = [];
		for(key in JSProblemState['cards']){
			cardNumbers.push(key);
		}

		var newCardNum = Math.max.apply(Math, cardNumbers) + 1	// We're adding the *next* card.

		// Add the new blank card
		cardSpace.append('<div id="card' + newCardNum + '" class="card ui-draggable"><a class="cardeditlink">edit</a><p class="cardtext">I am some example text.</p></div>');
			
		// Update the problem state to include this card.
		JSProblemState['cards'][String(newCardNum)] = 'Card ' + newCardNum;

		loadCards(cardSpace);

		// Give it a self-deletion button.
		//$('#card'+ newCardNum).append('<span class="deletegroup">x</span>');
		//$('.deletegroup').on('click tap', function(event){selfDelete(event, cardSpace)});
